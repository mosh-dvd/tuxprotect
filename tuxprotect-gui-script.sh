#!/bin/bash
# Tux Protect GUI Indicator Script (עם תפריט מלא)
...
# --- הוספנו פונקציות עזר מהקובץ המקורי ---
# הפונקציה הזו נדרשת כדי שהאפשרות "בדיקת בעיות" תעבוד
bus_corrector() {
    user=$(whoami)
	id=$(id -u "$user")
...
while true; do
    ...
    if [[ "$icon_to_show" != "$current_icon" && -n "$icon_to_show" ]]; then
        ...
        # נפעיל את bus_corrector כדי שהמשתנים יהיו זמינים
        bus_corrector
        
        # --- בניית התפריט המלא והפעלת הממשק הגרפי ---
        $GUI_BINARY --notification --no-middle --menu="$notification!/usr/share/tuxprotect/notification.sh
